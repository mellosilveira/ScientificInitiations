...
def calculate_baja_sweep(params: BajaParameters) -> Iterator[BajaSimRow]:
    """
    ...
    while current_h <= params.h_max + utils.EPSILON:
        # 1. Roll Moment (Nm)
        ...
        yield BajaSimRow(current_h, m_roll, d_fz, fz_ext, fz_int, m_sp)
...
def calculate_force_vs_angle(
    ...
    for d_ang in range(ang_min, ang_max + 1, step):
        ...
        if abs(cos_rad_sup) < utils.EPSILON or abs(cos_rad_inf) < utils.EPSILON:
            continue
            
        # F_axial = (Load * k) / cos(angle)
        f_ax_sup = (f_load * k_sup) / cos_rad_sup
        f_ax_inf = (f_load * k_inf) / cos_rad_inf
