...
from calculators.numerical_solvers import find_root
from calculators.trigonometry import rodrigues_rotation, cot, acot
...
def calculate_roll_center_parameters(geo: Suspension, vehicle_center_line: LineCoefficients3D, track_width: float) -> RollCenterResult:
    ...
    if roll_center is None:
        return RollCenterResult(front_instantaneous_center, None, None)
...
def calculate_alignment_metrics(
    ...
    return AlignmentMetricsResult(
        ...
        roll_center_parameters=calculate_roll_center_parameters(geo, vehicle_center_line, track_width),
...
def calculate_steering_metrics(geo: Suspension, wheelbase: float, inner_angle: float) -> SteeringMetricsResult:
    ...
    if abs(wheelbase) < EPSILON or abs(inner_angle) < EPSILON or R < EPSILON:
        return SteeringMetricsResult(ackermann_percentage=0.0, inner_angle=0.0, ideal_outer_angle=0.0, turning_radius=0.0, outer_angle=0.0)
