...
namespace IcVibracoes.Core.Operations.CalculateVibration.RigidBody
{
    /// <summary>
    ...
    public abstract class CalculateVibrationRigidBody<TRequest, TResponse, TResponseData, TInput> 
        : CalculateVibration<TRequest, TResponse, TResponseData, TInput>, ICalculateVibrationRigidBody<TRequest, TResponse, TResponseData, TInput>
        ...
        where TInput : RigidBodyInput, new()
    {
        ...
        protected CalculateVibrationRigidBody(ITime time)
        {
            this._time = time;
        }
        ...
        /// <returns></returns>
        public abstract double[] CalculateRigidBodyResult(TInput input, double time, double[] previousResult);
        ...
        /// <returns></returns>
        public abstract double[] BuildInitialConditions(TRequest request);
                    ...
                    try
                    {
                        ...
                        using (StreamWriter streamWriter = new StreamWriter(solutionPath))
                        {
                            ...
                            while (time <= input.FinalTime)
                            {
                                ...
                                result = this.CalculateRigidBodyResult(input, time, result);
                                ...
                                CompareValuesAndUpdateMaxValuesResult(result, maxValues);
                            }
                        }
                    }
                    catch (Exception ex)
        ...
        /// <returns></returns>
        private static void CompareValuesAndUpdateMaxValuesResult(double[] result, double[] maxValuesResult)
        {
        ...
        protected override async Task<TResponse> ValidateOperationAsync(TRequest request)
        {
            ...
            return response.AddErrorIf(() => request.DampingRatios.Any(v => v < 0), "Damping ratio cannot be less than zero.");
        }
    }
}
